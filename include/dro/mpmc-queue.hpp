...
private:
  ...
  alignas(cacheLineSize) std::size_t tailCache_ {0};
  alignas(cacheLineSize) std::atomic<std::size_t> head_ {0};
  alignas(cacheLineSize) std::size_t headCache_ {0};
...
public:
  ...
  template <typename... Args>
    requires std::constructible_from<T, Args...>
  bool try_emplace(Args&&... args) noexcept(MPMC_NoThrow_Type<T, Args&&...>)
  {
  ...
  bool try_push(const T& val) noexcept(MPMC_NoThrow_Type<T>)
  {
  ...
  template <typename P>
    requires std::constructible_from<T, P&&>
  bool try_push(P&& val) noexcept(MPMC_NoThrow_Type<T, P&&>)
  {
